"""
Mode 10 Video Assembler — таймлапс уборки пляжа (те же кроссфейды и микс аудио, что mode8).

Features:
- Quick crossfades (0.5-1s) between stages
- Звук волн / пляжа из клипов FastGen
- No TTS, no subtitles (pure visual satisfaction)
- Сохраняет звук из клипов FastGen
"""
...
def assemble_mode10_video(
    ...
    for p in video_paths:
        path = Path(p)
        if not path.exists():
            logger.warning(f"[Mode10 Assembler] Skip missing: {path}")
            continue
        vc = VideoFileClip(str(path))
        original_vcs.append(vc)

        # Save original audio from FastGen video
    ...
    if not clips:
        raise ValueError("[Mode10 Assembler] No valid video clips to assemble")

    logger.info(f"[Mode10 Assembler] Assembling {len(clips)} clips (T={T}s)")
    ...
    if speed_multiplier != 1.0:
        ...
        logger.info(f"[Mode10 Assembler] Applied {speed_multiplier}x speed for viral dynamics")
    ...
    if use_speed_ramping:
        try:
            ...
            logger.info("[Mode10 Assembler] Applied SPEED RAMPING: slow start → fast middle → slow end")
        except Exception as e:
            logger.warning(f"[Mode10 Assembler] Speed ramping failed, using constant speed: {e}")
    ...
    if final_hold_duration > 0:
        try:
            ...
            logger.info(f"[Mode10 Assembler] Added FINAL HOLD: {final_hold_duration}s freeze frame for retention")
        except Exception as e:
            logger.warning(f"[Mode10 Assembler] Final hold failed: {e}")
    ...
    if valid_audios:
        try:
            ...
            logger.info(f"[Mode10 Assembler] Preserved FastGen audio from {len(valid_audios)} clips (speed: {speed_multiplier}x)")
        except Exception as e:
    ...
    if combined_video_audio and bg_audio:
        ...
        logger.warning("[Mode10 Assembler] Audio: music only (FastGen audio was missing)")
    else:
        logger.warning("[Mode10 Assembler] No audio available")

    output_path.parent.mkdir(parents=True, exist_ok=True)
    logger.info(f"[Mode10 Assembler] Rendering -> {output_path}")
    ...
    logger.success(f"[Mode10 Assembler] Done -> {output_path}")
    return output_path
