"""
Mode 6 Video Assembler — Assemble cartoon drama videos.

Simple assembly with crossfades and background music.
No TTS, no subtitles — videos come with AI-generated audio from FastGen.
"""
...
def assemble_mode6_video(
    video_paths: list[Path | str],
    output_path: Path,
    title: str | None = None,
) -> Path:
    """
    Assemble final video from cartoon drama clips.

    FastGen videos already contain AI-generated character dialogue audio.
    We PRESERVE that audio at 100% volume and ADD quiet background music at 15%.
    No TTS, no subtitles.
    """
    ...
    for p in video_paths:
        path = Path(p)
        if not path.exists():
            logger.warning(f"[Mode6 Assembler] Skip missing: {path}")
            continue
        vc = VideoFileClip(str(path))
        original_vcs.append(vc)

        # Save original audio from FastGen video (character dialogue)
    ...
    logger.info(f"[Mode6 Assembler] Assembling {len(clips)} clips (T={T}s)")
    ...
    if valid_audios:
        try:
            ...
            logger.info(f"[Mode6 Assembler] Preserved FastGen audio from {len(valid_audios)} clips")
        except Exception as e:
            logger.warning(f"[Mode6 Assembler] Could not combine video audios: {e}")
            combined_video_audio = None
    ...
    try:
        ...
        music_path = _pick_background_music(topic="dramatic emotional intense", duration=final.duration)
        if music_path:
            ...
            bg = bg.with_effects([
                afx.MultiplyVolume(0.1),   # Very quiet — 7.5% so dialogue stays clear
                afx.AudioFadeIn(fade_dur),
                afx.AudioFadeOut(fade_dur),
            ])
            bg_audio = bg
            logger.info(f"[Mode6 Assembler] Added background music at 10%: {music_path.name}")
    except Exception as e:
        logger.warning(f"[Mode6 Assembler] Background music failed: {e}")

    # ─── Mix: FastGen dialogue (100%) + background music (15%) ───
    if combined_video_audio and bg_audio:
        final_audio = CompositeAudioClip([combined_video_audio, bg_audio])
        final = final.with_audio(final_audio)
        logger.info("[Mode6 Assembler] Audio: FastGen dialogue 100% + music 10%")
    elif combined_video_audio:
        final = final.with_audio(combined_video_audio)
        logger.info("[Mode6 Assembler] Audio: FastGen dialogue only (no music)")
    elif bg_audio:
        final = final.with_audio(bg_audio)
        logger.warning("[Mode6 Assembler] Audio: music only (FastGen audio was missing)")
    else:
        logger.warning("[Mode6 Assembler] No audio available")

    output_path.parent.mkdir(parents=True, exist_ok=True)
    logger.info(f"[Mode6 Assembler] Rendering → {output_path}")
    ...
    logger.success(f"[Mode6 Assembler] Done → {output_path}")
    return output_path
