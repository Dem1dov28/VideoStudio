"""
Mode 11 Video Assembler — Assemble monument timelapse videos.
Mode8-like path: crossfades, preserved FastGen audio, optional preview append.
"""
...
def assemble_mode11_video(
    ...
    preview_image_path: Path | str | None = None,
    preview_duration: float = 0.3,
) -> tuple[Path, float]:
    ...
    for p in video_paths:
        path = Path(p)
        if not path.exists():
            logger.warning(f"[Mode11 Assembler] Skip missing: {path}")
            continue
        ...
        clips.append(_clip_with_bottom_crop(vc, target_w, target_h, fps, bottom_crop))

    if not clips:
        raise ValueError("[Mode11 Assembler] No valid video clips to assemble")
    ...
    if valid_audios:
        try:
            ...
            if speed_multiplier != 1.0:
                ...
                combined_video_audio = combined_video_audio.with_effects([audio_fx.MultiplySpeed(speed_multiplier)])
            ...
            logger.warning(f"[Mode11 Assembler] Could not combine video audios: {e}")

    bg_audio = None
    try:
        from agents.video_editor.moviepy_editor import _pick_background_music
        music_path = _pick_background_music(topic="ambient epic cinematic", duration=final.duration)
        if music_path:
            ...
            bg = bg.with_effects([afx.MultiplyVolume(0.1), afx.AudioFadeIn(fade_dur), afx.AudioFadeOut(fade_dur)])
            bg_audio = bg
    except Exception as e:
        logger.warning(f"[Mode11 Assembler] Background music failed: {e}")

    if combined_video_audio and bg_audio:
        final = final.with_audio(CompositeAudioClip([combined_video_audio, bg_audio]))
