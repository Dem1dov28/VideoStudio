...
MONUMENT_TYPES = {
    "colosseum": ("КОЛИЗЕЙ", "COLOSSEUM", "🏟️"),
    "eiffel_tower": ("ЭЙФЕЛЕВА БАШНЯ", "EIFFEL TOWER", "🗼"),
    "great_wall": ("ВЕЛИКАЯ КИТАЙСКАЯ СТЕНА", "GREAT WALL", "🧱"),
    "giza_pyramids": ("ПИРАМИДЫ ГИЗЫ", "GIZA PYRAMIDS", "🔺"),
    "taj_mahal": ("ТАДЖ-МАХАЛ", "TAJ MAHAL", "🕌"),
    "christ_redeemer": ("ХРИСТОС-ИСКУПИТЕЛЬ", "CHRIST THE REDEEMER", "✝️"),
    "statue_of_liberty": ("СТАТУЯ СВОБОДЫ", "STATUE OF LIBERTY", "🗽"),
    "hanging_gardens": ("ВИСЯЧИЕ САДЫ", "HANGING GARDENS", "🌿"),
}

MONUMENT_TEMPLATES = [
    "{emoji} ОТ ИДЕАЛА ДО ПУСТОТЫ: {house_type} ЗА {duration} СЕКУНД!",
    "КАК ИСЧЕЗАЕТ {house_type} — ОБРАТНЫЙ TIMELAPSE {duration} СЕК",
    "{emoji} {house_type}: РАЗБОР ДО НУЛЯ ЗА {duration} СЕК",
]

MONUMENT_TEMPLATES_EN = [
    "{emoji} {house_type}: FROM ICON TO NOTHING IN {duration} SECONDS",
    "REVERSE TIMELAPSE: {house_type} DISAPPEARS IN {duration}S",
    "{emoji} {house_type} DECONSTRUCTED TO ZERO IN {duration}S",
]


def generate_clickbait_title(
    content_type: Literal["house", "vehicle", "monument"],
    style_or_type: str,
    location: str,
    ...
    if content_type == "house":
        ...
        templates = MONUMENT_TEMPLATES if language == "ru" else MONUMENT_TEMPLATES_EN
        type_mapping = MONUMENT_TYPES
        location_mapping = HOUSE_LOCATIONS
        default_emoji = "🏛️"
